...
//      print("Product name: \(String(describing: product.name))") //PB ep68 10mins
   ...
   //so we're going to check the product availability before we can set whether it is instock
      if product.quantity > 0 { //PB ep69 11mins
         ...
         if product.quantity < 5 { //PB ep69 13mins if quantity is less than 5 then we start showing how much is left, else just say it is in stock
            ...
            qtyLeftLabel.text = "Only \(product.quantity) \(qtyLeftString) left" //PB ep69 14mins meaning Only 2 items left or Only 1 item left
         }
      } else { //PB ep69 14mins
         ...
         addToCartButton.alpha = 0.5 //PB ep69 16mins
      }
